...
# 초기값 설정
status_message1 = '감지되지 않았습니다 (카메라 1).'

# 감지 통계 데이터
detection_data = []

# 카메라 객체 생성
camera1 = cv2.VideoCapture(0)  # 1번 카메라
...
@app.route('/')
def index():
    return render_template('index.html', status1=status_message1)

@app.route('/index2')
def index():
    return render_template('index2.html', status1=status_message1)

@app.route('/stat1')
def stats():
    return render_template('stat.html', status1=status_message1)
...
def generate_frames(camera, status_message):
    ...
    while True:
        ...
        if class_0_count > 0:
            ...
            event_info = {
                ...
                'camera_location': '대성동 목포시노인복지회관'  # 카메라 위치를 필요에 따라 수정하세요
            }
            root_ref.child('events2').push(event_info)
            event_counter += 1  # 이벤트 카운터 증가

            # 로그에 기록
            log_message = f'감지 이벤트: {class_0_count} 개의 객체 감지'
...
def camera1_feed():
    ...
    return Response(generate_frames(camera, status_message1),
